...
public class RedisClientTest {
    ...
    public void testSetGet() throws InterruptedException {
        ...
        RedisClientBase.connect("127.0.0.1", 6379)
                .onSuccess(new Block<RedisClientBase>() {
                    @Override
                    public void apply(final RedisClientBase client) {
                        System.out.println("connect succ");
                        client.send(new Command("SET", "test", "value"))
                                .onSuccess(new Block<Reply>() {
                                    @Override
                                    public void apply(Reply reply) {
                                        System.out.println("set suc");
                                        success.set(reply.data().equals("OK"));
                                        client.send(new Command("GET", "test"))
                                                .onSuccess(new Block<Reply>() {
                                                    @Override
                                                    public void apply(Reply reply) {
                                                        if (reply instanceof BulkReply) {
                                                            System.out.println(((BulkReply) reply).asAsciiString());
