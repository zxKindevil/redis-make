...
public class RedisClientBase {
    ...
    public static Promise<RedisClientBase> connect(String host, int port) throws InterruptedException {
        ...
        socketChannel.pipeline()
                .addLast(new RedisCommandEncoder(), new RedisReplyDecoder(),
        ...
        ChannelFuture connect = socketChannel.connect(new InetSocketAddress(host, port));

        connect.sync();
        System.out.println("done");

        connect.addListener(new ChannelFutureListenerPromiseAdapter<>(promise, client));
        return promise;
    }
